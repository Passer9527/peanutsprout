...
-- 0001_init: 初始结构：用户权限、连接、历史、审计、AI、迁移、看板、任务、插件、设置
-- 0002_audit_drop_user_fk: 审计日志改为不可变：移除 audit_logs.user_id 外键（原 ON DELETE SET NULL 会让删除用户打断哈希链）
-- 0003_audit_hash_version: 审计哈希版本化：新增 audit_logs.hash_version，老行按 v1 校验、新行按 v2（纳入 IP/UA/耗时并区分 null 与空串）
